...
export default function ProfileScreen() {
  const { nameAr, roleAr, joinedAr, avatarInitial, stats, activityWeek } = DEMO_PROFILE;
    ...
    <div className="w-full pb-6" dir="rtl">
      ...
      <div className="px-4 pt-4 pb-4" style={{ borderBottom: '1px solid var(--border-subtle)' }}>
        <div className="flex items-center gap-3">
          {/* Avatar */}
          <div className="w-14 h-14 rounded-2xl flex items-center justify-center font-arabic text-xl font-bold flex-shrink-0"
            style={{ background: 'rgba(212,137,30,0.15)', border: '1px solid rgba(212,137,30,0.28)', color: 'var(--accent)' }}>
            {avatarInitial}
          </div>
          <div>
            <h2 className="font-arabic text-base font-bold" style={{ color: 'var(--text-primary)' }}>{nameAr}</h2>
            <p className="font-arabic text-xs mt-0.5" style={{ color: 'var(--text-muted)' }}>{roleAr}</p>
            <p className="font-arabic text-xs" style={{ color: 'var(--text-muted)', fontSize: '10px' }}>{joinedAr}</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-2 px-4 pt-4 pb-3">
        {stats.map(({ label, value }) => (
          <div key={label} className="rounded-xl p-3"
            ...
            <p className="font-arabic text-lg font-bold stat-number" style={{ color: 'var(--text-primary)' }}>{value}</p>
          ...
          {activityWeek.map((count, i) => {
            ...
            return (
              <div key={i} className="flex-1 flex flex-col items-center gap-1">
                ...
                <span className="font-arabic" style={{ fontSize: '9px', color: isToday ? 'var(--accent)' : 'var(--text-muted)' }}>
                  {DAYS_AR[i]}
                </span>
              </div>
            );
